...
<table id="TABLE_KSZ_MGQ_CBC"><thead><tr><th>
...
To set the <code>SameSite</code> cookie attribute if required for web browser support.
...
For security reasons, the <code>SameSite</code> attribute of the HTTP cookie may need to be set for the Web Browser to accept the HTTP session cookie.
...
<code>javaPath=<path-to-JDK></code>

Enter the full path to the Java binary. For example:

-   Linux: <code>javaPath=/usr/lib/jvm/java-21/bin/java</code>

-   macOS: <code>javaPath=/Library/Java/JavaVirtualMachines/jdk-21/Contents/Home/bin/java</code>

-   Windows: <code>javaPath=C:\java\jdk-21\bin\java</code>


</td></tr><tr><td>

To specify a custom logging configuration file so you can control the logging information that WebLogic Remote Console collects.
...
Table 1. Tasks in Each Perspective. Identifies which perspective is responsible for different management tasks.
...
Table 2. Access Limitations based on Role. Some examples of the limitations based on role.
...
<table id="TABLE_MPM_VYN_SDC"><thead><tr><th>
...
Disable the **Production Mode** option to convert your domain to development mode.
...
Enable the **Production Mode** option to convert your domain to production mode. {{< alert title="Note" color="primary" >}} As of WebLogic Server 14.1.2.0.0, when you select production mode, *secured* production mode is enabled by default.

If you want to apply the features of basic production mode only, then you must explicitly disable the **Secured Production Mode** option. If you use Node Manager to start your Managed Servers, then you must also add the <code>-Dweblogic.securemode.SecureModeEnabled=false</code> startup argument to each Managed Server. See [Configure Startup Arguments for a Managed Server](#GUID-C1986FE5-E41C-41CC-880E-D6C78A691A7C).
...
Enable the **Production Mode** and **Secured Production Mode** options.**Note**: For more information, see [Using Secured Production Mode](https://docs.oracle.com/pls/topic/lookup?ctx=en/middleware/fusion-middleware/weblogic-remote-console/administer&id=SECMG-GUID-9ED2EF38-F763-4999-80ED-27A3FBCB9D7D) in **Administering Security for Oracle WebLogic Server**.
...
## Server and Service Migration {#GUID-E4B7E9F8-49E0-4FB3-BCA9-BF3470E3E167}

You can configure WebLogic Server to automatically and seamlessly migrate whole server instances to new machines or migrate services to new servers in the event of a server failure. You can also migrate services or servers manually as needed.

See [Whole Server Migration](https://docs.oracle.com/pls/topic/lookup?ctx=en/middleware/fusion-middleware/weblogic-remote-console/administer&id=CLUST-GUID-BD940A79-CF6E-4FE6-9922-85220AE835B4) and [Service Migration](https://docs.oracle.com/pls/topic/lookup?ctx=en/middleware/fusion-middleware/weblogic-remote-console/administer&id=CLUST-GUID-AEECC92D-88DA-4E07-8C42-A24DBC1C7076) in **Administering Clusters for Oracle WebLogic Server**.

### Configure Whole Server Migration {#GUID-019DA8C3-B231-43A0-A0E3-5375725862DD}

You can configure clusters and Managed Servers within clusters to automatically migrate all of their services to a new machine if a failure is detected on their original physical machine.

For guidance on how to set up your servers and clusters for whole server migration, see [Whole Server Migration](https://docs.oracle.com/pls/topic/lookup?ctx=en/middleware/fusion-middleware/weblogic-remote-console/administer&id=CLUST-GUID-BD940A79-CF6E-4FE6-9922-85220AE835B4) in **Administering Clusters for Oracle WebLogic Server**.

{{< alert title="Note" color="primary" >}}



If you want to migrate services that are pinned to a server, see [Configure Service Migration](#GUID-DF76BC28-F003-48B6-A934-DAFE62C46E85) instead.

{{< /alert >}}


1.  Make sure your environment is properly configured for whole server migration. Generally, this involves setting up Managed Servers, clusters, machines, data sources, and Node Manager. See [Preparing for Automatic Whole Server Migration](https://docs.oracle.com/pls/topic/lookup?ctx=en/middleware/fusion-middleware/weblogic-remote-console/administer&id=CLUST-GUID-ADD6E564-5CD4-42EB-963B-352DCC7EE8AA) and [Configuring Automatic Whole Server Migration](https://docs.oracle.com/pls/topic/lookup?ctx=en/middleware/fusion-middleware/weblogic-remote-console/administer&id=CLUST-GUID-FC90C9BB-AF93-453A-91FF-A861B42E1442) in **Administering Clusters for Oracle WebLogic Server**.

2.  In the **Edit Tree**, go to **Environment**, then **Clusters**, and select the cluster where you want to enable whole server migration.

3.  On the **Migration** tab, update values as necessary for your environment and click **Save**.

4.  On the **Candidates** subtab, under **Candidate Machines for Migratable Servers**, select all of the machines that you want available for migration and move them over to the **Chosen** side.

5.  Click **Save**.

6.  Still in the **Edit Tree**, go to **Environment**.

    -   If the cluster is a dynamic cluster, then go to **Server Templates**, select the server template that applies its settings to the cluster's server members and proceed to the next step.
    -   If the cluster consists of configured Managed Servers, then go to **Servers**, select a server within the cluster where you just enabled whole server migration and proceed to the next step.
    If the cluster is a mixed cluster, with both configured and dynamic servers, you'll need to configure migration settings on both the server template as well as on any configured server instances.

7.  On the server template or server's **Migration** tab, turn on **Automatic Server Migration Enabled**.

8.  **Optional**: If you want a configured server to only use a subset of the candidate machines specified by the cluster, then in the **Candidate Machines** field, move the machines you want to be available for migrated to, to the **Chosen** side.

9.  Click **Save**.

10. Repeat on every configured server in the cluster where you want to enable migration.


Your domain is now set up to automatically migrate a server and its services to another machine upon failure.

If you want to manually migrate a server, see [Manually Migrate Servers and Services](#GUID-15AD4516-7659-42AA-A436-3CE7A689CF1D).

### Configure Service Migration {#GUID-DF76BC28-F003-48B6-A934-DAFE62C46E85}

You can configure services that are pinned to a single server to automatically migrate from their original server instance to a new server instance \(within the same cluster\) in the event of failure.

You can migrate the following pinned services:

-   JMS-Related Services
-   JTA Transaction Recovery Services
...
If you want to migrate a server instance and *all* of its services, see [Configure Whole Server Migration](#GUID-019DA8C3-B231-43A0-A0E3-5375725862DD) instead.

{{< /alert >}}


For guidance on how to set up your WebLogic Server environment to automatically migrate services, see [Service Migration](https://docs.oracle.com/pls/topic/lookup?ctx=en/middleware/fusion-middleware/weblogic-remote-console/administer&id=CLUST-GUID-AEECC92D-88DA-4E07-8C42-A24DBC1C7076) in **Administering Clusters for Oracle WebLogic Server**, or more specifically:

-   [Roadmap for Configuring Automatic Migration of JMS-Related Services](https://docs.oracle.com/pls/topic/lookup?ctx=en/middleware/fusion-middleware/weblogic-remote-console/administer&id=CLUST-GUID-F313A5C5-666A-4BFD-A7E1-6213EBA3B02B)

-   [Roadmap for Configuring Automatic Migration of the JTA Transaction Recovery Service](https://docs.oracle.com/pls/topic/lookup?ctx=en/middleware/fusion-middleware/weblogic-remote-console/administer&id=CLUST-GUID-F8F9E59A-766E-43BD-BF4C-F21A37F70798)

-   [Automatic Migration of User-Defined Singleton Services](https://docs.oracle.com/pls/topic/lookup?ctx=en/middleware/fusion-middleware/weblogic-remote-console/administer&id=CLUST-GUID-1E5ED7D6-FF33-429F-9072-543B8BF7222A)


If you want to manually migrate a service, see [Manually Migrate Servers and Services](#GUID-15AD4516-7659-42AA-A436-3CE7A689CF1D).

### Manually Migrate Servers and Services {#GUID-15AD4516-7659-42AA-A436-3CE7A689CF1D}

You can manually migrate services to new server instances or migrate servers to new machines as needed.

Make sure your environment is properly configured for migration. See [Server and Service Migration](#GUID-E4B7E9F8-49E0-4FB3-BCA9-BF3470E3E167).

1.  In the **Monitoring Tree**, go to **Environment**, then **Migration** and select the tab that applies to the type of service or server that you want to migrate.

2.  For JMS-related services:
    ...
    4.  Repeat for every JMS service that you want to migrate.

3.  For JTA transaction recovery services:
    ...
    4.  Repeat for every JTA service that you want to migrate.

4.  For user-defined singleton services:
    ...
    4.  Repeat for every singleton service that you want to migrate.
...
5.  For whole servers:
    ...
    You can only migrate servers that are currently running and have **Automatic Server Migration Enabled** enabled.
    ...
    4.  Repeat for every server that you want to migrate.

6.  To monitor the status of migration requests, go to **Environment**, then **Migration**.

    -   For services, go to **Service Migration Data Runtimes**
    -   For whole server, go to **Migration Data Runtimes**

Following migration, you should go to the **Edit Tree** and open the Shopping Cart to check if there are any pending changes that need to be committed.
