 ...
 */
public abstract class ProviderMBeanCustomizer {

  private static final Logger LOGGER = Logger.getLogger(ProviderMBeanCustomizer.class.getName());

  private static final String LDAP_AUTHENTICATOR =
    "weblogic.security.providers.authentication.LDAPAuthenticator";

  private static final Map<String, String> LDAPAuthenticatorDescriptionToType = new HashMap<>();
  ...
  public static JsonObject getType(
    @Source(property = "type") JsonObject type,
    @Source(property = "providerClassName") JsonObject providerClassName
  ) {
    if (type != null) {
    ...
    if (providerClassName == null) {
      // This is an optional singleton that currently doesn't exist, like the adjudicator.
      // Return null to indicate that the 'type' property isn't available.
      return null;
    }

    // Try to map the provider's providerClassName to its type.
    String value = providerClassNameToType.get(ExpandedValue.getStringValue(providerClassName));
    if (value == null) {
      throw
        new AssertionError(
          "Unknown providerClassName "
          + ExpandedValue.getStringValue(providerClassName)
        );
    }

    return ExpandedValue.fromString(value).set(false).getJson();
  }
}
