 ...
 * Just a class name
 * Custom code for UserPasswordCredentialMapperEJBMBean
 */
public class UserPasswordCredentialMapperEJBMBeanCustomizer {

  private UserPasswordCredentialMapperEJBMBeanCustomizer() {
  }
  ...
  private static class CustomizedCreateHelper extends CreateHelper {
    ...
    protected Response<String> getKey(InvocationContext ic, List<FormProperty> properties) {
      ...
      Response<String> ejbResponse = findRequiredStringProperty(ic, new Path("EJB"), properties);
      if (!ejbResponse.isSuccess()) {
        return response.copyUnsuccessfulResponse(ejbResponse);
      }
      return response.setSuccess("module=" + moduleResponse.getResults() + ", ejb=" + ejbResponse.getResults());
    }
  }
}
