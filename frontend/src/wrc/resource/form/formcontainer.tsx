...
export const FormContainer = ({ model, pageContext }: Props) => {
  ...
  const actionSelected = async (action: Action): Promise<void> => {
    const formModel = contentModel as FormContentModel;
    ...
    if (action.saveFirstLabel && formModel.hasChanges()) {
      try {
        ...
        const messageResponse: Response = await parseResponseJson<Response>(response);
