...
import { doAction, parseResponseJson } from "wrc/shared/model/transport";
...
const FormToolbar = ({ formModel, setModel, showHelp, onHelpClick, pageContext, pageLoading = false, onPageRefresh }: Props) => {
  {
    ...
    const [backTarget, setBackTarget] = useState<string | undefined>();

    useEffect(() => {
      const databus = ctx?.databus;
      if (!databus) return;

      const signal = databus.subscribe((event: StatusData) => {
        const nextBackTarget = event?.contexts?.main?.["back-resource-data"];
        setBackTarget((previousBackTarget) => nextBackTarget ?? previousBackTarget);
      });
    ...
    const getMainCreateCancelTarget = () => {
      if (backTarget) return backTarget;

      const breadcrumbs = formModel.getBreadcrumbs?.() || [];
      if (breadcrumbs.length > 1) {
        return breadcrumbs[breadcrumbs.length - 2]?.resourceData;
      }
    ...
    const cancelChanges = () => {
      ...
      if (!formModel.isCreate()) return;

      if (pageContext === "main") {
        const target = getMainCreateCancelTarget();
        if (target) {
          ctx?.context?.routerController?.navigateToAbsolutePath(target);
        }
        return;
      }

      ctx?.context?.routerController?.navigateToAbsolutePath(
        formModel.getSelfResourceData() || ""
      );
    };
          ...
          if (shouldShowCreateCTA) {
            ...
            if (createFormRef) {
              ...
              try {
                ...
                const messageResponse: any = await parseResponseJson<Response>(res).catch(() => undefined);
                if (messageResponse) {
                  broadcastMessageResponse(ctx, messageResponse);
