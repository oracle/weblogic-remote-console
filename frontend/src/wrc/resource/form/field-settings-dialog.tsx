...
export default function FieldSettingsDialog({
  ...
  const isIndirectField = useMemo(() => {
    ...
    return isIndirectProperty(formModel, fieldDescription.name);
  }, [fieldDescription, formModel]);
  const indirectSourceValue = fieldDescription ? getIndirectSourceValue(formModel, fieldDescription.name) : undefined;
  ...
  useEffect(() => {
    ...
    if (isIndirectField) {
      ...
      const fetchIndirectData = async () => {
        try {
          if (typeof indirectSourceValue === "string" && indirectSourceValue) {
  ...
  }, [fieldDescription, formModel, isIndirectField, indirectSourceValue]);
  ...
  const supportsTokens = !isIndirectField && formModel.canSupportTokens && !!fieldDescription.supportsModelTokens;
  const supportsUnresolvedRef = !isIndirectField && (
                ...
                <div class="wrc-indirect-field-container">
                  ...
                  {!indirectLoading && indirectDisplayValue && (
                    <oj-c-button
                      ...
                      aria-label={t["wrc-indirect-field"].copy.button.tooltip}
                    >
                    ...
                    <oj-c-button
                      ...
                      onojAction={async () => {
                        try {
                          ...
                          const res = await electronAPI.getOpenFile({ filepath: (regularValue as string) || "" });
                          if (res?.succeeded && res?.filePath) setRegularValue(res.filePath);
                        } catch (_e) {}
                      }}
                    >
                      <span slot="startIcon" class="oj-ux-ico-upload"></span>
