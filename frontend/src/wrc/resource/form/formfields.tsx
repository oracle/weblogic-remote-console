...
import MultiSelect, { ChangeEvent, SelectOption } from "../../multiselect";
...
import { PropertyValue, Reference } from "../../shared/typedefs/rdj";
...
const isIndirectProperty = (formModel: FormContentModel, propertyName: string): boolean => {
  if (typeof (formModel as any).isIndirectProperty === "function") {
    return (formModel as any).isIndirectProperty(propertyName);
  }

  const propertyData = (formModel as any)?.rdj?.data?.[propertyName] as { indirect?: boolean | string } | undefined;
  const indirect = propertyData?.indirect;

  return indirect === true || (typeof indirect === "string" && indirect.toLowerCase() === "true");
};

const getIndirectSourceValue = (formModel: FormContentModel, propertyName: string): string | undefined => {
  const propertyData = (formModel as any)?.rdj?.data?.[propertyName] as { value?: unknown } | undefined;

  return typeof propertyData?.value === "string" ? propertyData.value : undefined;
};
...
const DefaultedField = () => (
  <oj-input-text
    id="why"
    class="cfe-form-input-integer-sm wrc-defaulted-field"
...
const IndirectField = ({
  ...
  const indirectSourceValue = getIndirectSourceValue(formModel, fieldDescription.name);
  ...
  useEffect(() => {
    ...
    const fetchIndirectValue = async () => {
      try {
        if (typeof indirectSourceValue === "string" && indirectSourceValue && isMounted) {
          const data = await getDataComponentText(indirectSourceValue);
  ...
  }, [fieldDescription?.name, indirectSourceValue]);
...
export const getInputField = (
  ...
  const isIndirectField = isIndirectProperty(formModel, fieldDescription.name);
  ...
  return (
    <div class='oj-flex oj-flex-item oj-sm-12'>
      <span class={`wrc-value-group ${formModel.hasPropertyChanged(fieldDescription.name) && !formModel.isReadOnly(fieldDescription) && !formModel.isDisabled(fieldDescription) ? 'wrc-field-changed' : ''} ${formModel.isReadOnly(fieldDescription) ? 'wrc-align-center' : ''}`}>
        ...
        {!isDefaulted
          ? (isIndirectField
            ? <IndirectField fieldDescription={fieldDescription} formModel={formModel} />
            : getOjTextArea())
          : <DefaultedField />}
      </span>
    </div>
...
const getTableEditor = (options: TableEditorOptions) => {
  ...
  const isPropertiesEditor = fieldDescription.type === "properties";
  ...
  return (
    <div class={`OLDcfe-form-field oj-flex oj-flex-item ${isPropertiesEditor ? 'wrc-properties-editor' : ''}`}>
      <span class={`wrc-value-group ${isPropertiesEditor ? 'wrc-properties-editor-value-group' : ''} ${formModel.hasPropertyChanged(fieldDescription.name) && !formModel.isReadOnly(fieldDescription) && !formModel.isDisabled(fieldDescription) ? 'wrc-field-changed' : ''}`}>
        <RestartNeededImage
          fieldDescription={fieldDescription}
          formModel={formModel}
        />
...
export const getFileSelectorForFilename = (
  ...
  const isElectron = !!(window as any).electron_api;

  const chooseFileClickHandler = async () => {
    if (isElectron) {
      const electronAPI = (window as any).electron_api as ElectronAPI;
      const result = await electronAPI.getOpenFile({ filepath: (formModel.getProperty(fieldDescription.name) as string) || '' });

      if (result?.succeeded && result?.filePath) {
        formModel.setProperty(fieldDescription.name, result.filePath);
        setModel(formModel.clone());
      }
      return;
    }
  ...
  return (
    <div class="OLDcfe-form-field oj-flex oj-flex-item">
      <span class={`wrc-value-group ${formModel.hasPropertyChanged(fieldDescription.name) && !formModel.isReadOnly(fieldDescription) && !formModel.isDisabled(fieldDescription) ? 'wrc-field-changed' : ''} ${formModel.isReadOnly(fieldDescription) ? 'wrc-align-center' : ''}`}>
        ...
        {isElectron ? (
          <oj-c-button chroming='borderless' onojAction={chooseFileClickHandler} aria-label="Choose directory and filename">
            <span slot='startIcon' class='oj-ux-ico-upload'></span>
          </oj-c-button>
        ) : null}
...
export const getFileSelectorForNewFile = (
  ...
  const chooseFileClickHandler = async () => {
    ...
    if ((window as any).electron_api) {
      ...
      if (file?.succeeded && file.filePath) {
