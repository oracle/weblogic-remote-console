...
import { parseResponseJson } from "wrc/shared/model/transport";
import { isHrefCellData } from "./href-cell";
import { ProxyDownloadLink } from "../../shared/proxy-download-link";
import { buildTableCellValue, getTableCellTextClassName, TableCellValue } from "./table-cell";
...
const Table = ({ tableContent, pageContext, bare, onSelectionChanged }: Props) => {
  ...
  const cleanupSelected = (rowArray: Record<string, TableCellValue>[]) => {
  ...
  const setupRowDataProvider = () => {
    ...
    rows?.forEach((row) => {
      const cell: Record<string, TableCellValue> = {};

      displayColumns?.forEach((column) => {
        const columnId = column["id"];
        const columnDefinition = columnDefinitionsByName.get(columnId || "");

        if (columnId) {
          cell[columnId] = buildTableCellValue(columnDefinition, row[columnId]);
        }
      });
  ...
  const setupColumns = () => {
    ...
    columnDefinitionsByName = new Map(
      (rawColumns || []).map((column) => [column.name, column] as [string, Column]),
    );
  ...
  const actionHandler = async (action: Action): Promise<void> => {
    ...
    await tableContent
      ...
      .then((response) => parseResponseJson<Response>(response))
  ...
  const cellRenderer = ( cell: ojTable.CellTemplateContext<any, any>) => {
    const columnKey = cell.columnKey as string | undefined;
    const rowData = cell.item?.data as Record<string, TableCellValue> | undefined;
    const cellValue = (columnKey && rowData) ? rowData[columnKey] : cell.data;
    const columnDefinition = columnKey ? columnDefinitionsByName.get(columnKey) : undefined;

    if (isHrefCellData(cellValue)) {
      return (
        <ProxyDownloadLink
          request={{
            url: cellValue.url,
            filename: cellValue.downloadFilename,
            mimeType: cellValue.mimeType,
          }}
          onError={(error) => broadcastErrorMessage(ctx, error)}
        >
          {cellValue.label}
        </ProxyDownloadLink>
      );
    ...
    return (
      <span class={getTableCellTextClassName(columnDefinition)}>
        {cellValue as any}
      </span>
    );
  }
