...
jest.mock("wrc/shared/model/common", () => {
  ...
  const a = {
    getActions: jest.fn().mockReturnValue(fakeActions),
...
describe("Actions component", () => {
  ...
  it("should keep the pending action set until the action promise settles", async () => {
    let resolveAction!: () => void;
    const setPendingActionName = jest.fn();
    ...
    const actionPromise = invokePendingAction({
      pendingActionName: undefined,
      nextPendingActionName: "action1",
    ...
    expect(setPendingActionName).toHaveBeenNthCalledWith(1, "action1");
