import { PDJ } from "wrc/shared/typedefs/pdj";
...
describe("FormContentModel", () => {
  let rdj: RDJ;
  let pdj: PDJ;
  let formContentModel: FormContentModel;
  ...
  describe("accessor tests", () => {
    describe("test boolean operations", () => {
      ...
      it("should replace with a wdt token", () => {
        ...
        expect(formContentModel.getProperty("aboolean")).toEqual(
          "@@MY:TOKEN@@",
        );
      });
    ...
    describe("test string operations", () => {
      ...
      it("should replace string with wdt token", () => {
        ...
        expect(formContentModel.getProperty("astring")).toEqual(
          "@@MY:TOKEN2@@",
        );
      });
      ...
      it("should use top-level PDJ presentation singleColumn for inline forms", () => {
        const inlineRdj: RDJ = {
          navigation: "nav",
          self: { resourceData: "self" },
          breadCrumbs: [],
        ...
        const inlineModel = new FormContentModel(inlineRdj, inlinePdj);

        expect(inlineModel.getNumberOfColumns()).toBe(1);
      });
    });
    ...
    describe("test reference-dynamic-enum operations", () => {
      ...
      it("should get value from rdj", () => {
        expect(formContentModel.getProperty("aRefEnum")).toEqual(
      ...
      it("should set an unresolved reference", () => {
        formContentModel.setPropertyAsUnresolvedReference(
          "aRefEnum",
          "unresolved ref",
        );
        expect(formContentModel.getProperty("aRefEnum")).toEqual(
  ...
  describe("test update function", () => {
    ...
    it("shouldn't update values with no changes", async () => {
    ...
    it("should update with changes", async () => {
      ...
      // Order doesn't matter in the data payload, but this test assumes an order..
      // might need to be made more flexible in the future
      expect(global.fetch).toHaveBeenCalledWith(
        expect.any(String),
        expect.objectContaining({
          body: JSON.stringify({
    ...
    it("should update without changes that were redundant", async () => {
      ...
      // This is the same value that is in the rdj, should be thrown out
      formContentModel.setProperty("aRefEnum", {
        label: "opt1",
        resourceData: "/api/something/opt1",
      });
