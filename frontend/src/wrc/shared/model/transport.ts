...
import { tryGetDatabus } from "../../integration/databus-accessor";
...
import { buildUrl } from "../backend-url";
...
type ResponseLikeError = Error & {
  body?: string;
  status?: number;
  url?: string;
};

const createUniqueHeaders = () => {
...
const normalizeResponseBody = (bodyText?: string): string | undefined => {
  const trimmed = bodyText?.trim();
  if (!trimmed) return undefined;

  const withoutMarkup = trimmed.replace(/<[^>]*>/g, " ");
  const normalized = withoutMarkup.replace(/\s+/g, " ").trim() || trimmed;
...
const buildResponseError = (
  ...
  const target = requestUrl || response.url || "request";
  const statusText = `HTTP ${response.status}${response.statusText ? ` ${response.statusText}` : ""}`;

  let message: string;
  if (body) {
    message = response.ok
      ? `Unexpected non-JSON response from ${target}: ${body}`
      : `${statusText} from ${target}: ${body}`;
  } else if (response.ok) {
    message = `Unexpected empty or non-JSON response from ${target}`;
  } else {
    message = `${statusText} from ${target}`;
  }

  const error = new Error(message) as ResponseLikeError;
...
export const parseResponseJson = async <T>(
  response: Response,
  requestUrl?: string,
): Promise<T> => {
  ...
  if (!bodyText.trim()) {
    throw buildResponseError(response, requestUrl, bodyText);
  }
...
const getDataComponentRaw = async (url: string, context?: string) => {
  ...
  if (!res.ok) {
    ...
    throw buildResponseError(res, fixedUrl, bodyText);
  }
...
export const downloadResource = async ({
  ...
  const response = await getDataComponentRaw(url);
  const responseBlob = await response.blob();
  const blob =
    mimeType && responseBlob.type !== mimeType
      ? new Blob([responseBlob], { type: mimeType })
      : responseBlob;
  saveBlobToFile(blob, filename || "download");
};
...
export const getDataComponent = async <T = unknown>(url: string, context?: string): Promise<T> => {
  return parseResponseJson<T>(await getDataComponentRaw(url, context), url);
};
