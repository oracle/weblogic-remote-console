...
export class FormContentModel extends Model {
  ...
  getNumberOfColumns(): number {
    return (this.pdj.presentation?.singleColumn || this.pdj.form?.presentation?.singleColumn) /*||
      this.pdj.createForm ||
  ...
  getWidth(fieldDescription: Property): string | undefined {
    ...
    if (!width && this.rdj?.data && !Array.isArray(this.rdj.data)) {
      const propertyData = (this.rdj.data as any)[fieldDescription.name] as PropertyValueHolder;
      const indirect = propertyData?.indirect as boolean | string | undefined;

      if (indirect === true || (typeof indirect === "string" && indirect.toLowerCase() === "true")) {
        return 'xxl';
      }
    }
