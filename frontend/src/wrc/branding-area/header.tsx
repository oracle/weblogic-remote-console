...
export function BrandingHeaderImpl({context}: Props) {
  ...
  const navtreeToggleClick = () => {
    try {
      ...
      if (drawer.startOpened) {
        void initializeDrawerResizerWhenReady();
      }
    } catch (_e) {
      // ignore
    }
  ...
  const initDrawerResizer = (storedWidth?: number | string) => {
    ...
    // Avoid stacking duplicate listeners when the drawer is reopened.
    if (pane.dataset.wrcResizerInitialized === "true") {
      return;
    }
    pane.dataset.wrcResizerInitialized = "true";
  ...
  const initializeDrawerResizerWhenReady = async () => {
    adjustDrawerLayoutHeight();
    ...
    try {
      const busyContext = Context.getContext(drawer).getBusyContext();
      await busyContext.whenReady();
    } catch (_e) {
      // If BusyContext lookup fails, continue with best-effort DOM probing below.
    }

    const storedWidth = await getStoredWidth();

    let attempts = 0;
    const tryInitialize = () => {
      const upgradedDrawer = document.getElementById('drawer-layout') as any;
      const pane = document.getElementById('navtree-container2') as HTMLElement | null;
      const resizer = document.getElementById('drawer-start-resizer') as HTMLElement | null;
      if (upgradedDrawer?.startOpened && pane && resizer) {
        initDrawerResizer(storedWidth);
        return;
      }
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(initializeDrawer);
    window.addEventListener('resize', adjustDrawerLayoutHeight);

    return () => {
      cancelled = true;
      cancelAnimationFrame(frame);
      window.removeEventListener('resize', adjustDrawerLayoutHeight);
    };
  }, []);
...
DataProviderManager.getCapabilities().then(caps => {
  ...
  */
  ...
  getDataComponent<AboutResponse>("/api/about").then((response) => {
    Logger.info(response);

    const caps = response?.about?.capabilities;
