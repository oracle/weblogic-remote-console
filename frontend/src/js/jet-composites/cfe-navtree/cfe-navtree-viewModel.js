...
define(['knockout', 'ojs/ojcontext', 'ojs/ojknockout-keyset', 'wrc-frontend/integration/controller', 'wrc-frontend/microservices/navtree/navtree-manager', 'wrc-frontend/microservices/perspective/perspective-manager', 'wrc-frontend/microservices/perspective/perspective-memory-manager', 'wrc-frontend/core/runtime', 'wrc-frontend/integration/viewModels/utils', 'wrc-frontend/core/utils', 'wrc-frontend/core/types', 'ojs/ojknockout', 'ojs/ojnavigationlist', 'ojs/ojdialog'],
  function (ko, Context, keySet, Controller, NavtreeManager, PerspectiveManager, PerspectiveMemoryManager, Runtime, ViewModelUtils, CoreUtils, CoreTypes) {
    ...
    function NavtreeViewModel(context) {
      ...
      this.connected = function () {
        ...
        keys.sort((e1, e2) => { e1.split ('/').length - e2.split('/').length });
        if (keys.length > 0) {
          keys.forEach((key) => {
      ...
      .finally(() => {
        ...
        .catch((response) => {
          ...
          if (treeaction.update) {
            ...
            self.expanded.add([iterPath]);
            iterPath += '/';
          });
          }

          populateExpandedSet.call(this, this.expanded());
        }
      });
      ...
      this.onSelect = function (event) {
        ...
        if (nodeId !== null && nodeId !== '') {
          ...
          if (CoreUtils.isNotUndefinedNorNull(node)) {
            ...
            if (CoreUtils.isNotUndefinedNorNull(resourceData) && isRenderableNode(resourceData)) {
              const index = resourceData.indexOf('/data/');
              if (index !== -1) {
                self.perspectiveMemory.contentPage.resourceDataFragment = resourceData.substring(index + 1);
              }
      ...
      function isRenderableNode(resourceData) {
        return (Runtime.getRole() === CoreTypes.Console.RuntimeRole.TOOL.name ? resourceData !== decodeURIComponent(router.observableModuleConfig().params.ojRouter.parameters.path()): true);
      }

      /**
       * Populate ``observable`` whose value is a ``KeySet`` from nodes managed by navtreeManager associated with this cfe_navtree JET Composite instance.
       * @param {ObservableKeySet} expandedSet
       * @private
       */
      function populateExpandedSet(expandedSet) {
        this.busyContext.whenReady(10000)
          .then(() => {
          const resolve = this.addBusyState();
        this.navtreeManager.populateNodeSet(expandedSet)
          .finally(() => {
          resolve();
      });
      });
