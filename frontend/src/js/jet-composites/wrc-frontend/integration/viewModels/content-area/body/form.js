...
//MLW          selector.val(initialValue);
            if (!isEditing()) {
              self.createForm.backingDataAttributeValueChanged(name, initialValue);
            }
//MLW          self[`${PageDefinitionCommon.FIELD_VALUES}${name}`](initialValue);
            if (!self.dirtyFields.has(name)) self.dirtyFields.add(name);
/* AL           viewParams.signaling.popupMessageSent.dispatch({
                blocking: true,
...
*/
            ...
            if (addToArchiveSwitchValue && newValue !== '') {
              ...
              //If the source path is blanked in edit when the archive swith is ON
          ...
          if (isWdtForm() && window?.api?.ipc &&
            ...
            if (index !== -1) {
              ...
              if (index1 === -1) {
                ...
                const value = self.perspectiveMemory.getAddToArchiveSwitchValue.call(self.perspectiveMemory, 'SourcePath');
            ...
            if (index !== -1) {
              ...
              if (index1 === -1) {
                ...
                property.required = false;
                pdjData.sliceForm.properties.splice(index, 0, property);
                const value = self.perspectiveMemory.getAddToArchiveSwitchValue.call(self.perspectiveMemory, 'PlanPath');
        ...
        function setAddToArchiveSwitchValue(fieldName, value) {
          ...
          // tabs, in the WKT-UI.
          self.perspectiveMemory.setAddToArchiveSwitchValue.call(self.perspectiveMemory, fieldName, value);
        }
