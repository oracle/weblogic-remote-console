...
define(['knockout',  'ojs/ojarraydataprovider', 'wrc-frontend/apis/data-operations', 'wrc-frontend/apis/message-displaying', 'wrc-frontend/core/runtime', 'wrc-frontend/microservices/page-definition/utils', 'wrc-frontend/core/cbe-types', 'wrc-frontend/core/cbe-utils', 'wrc-frontend/core/types', 'wrc-frontend/core/utils', 'ojs/ojlogger', 'ojs/ojknockout'],
        ...
        viewParams.onCanExit(eventType)
          .then(result => {
            ...
            if (Runtime.getRole() === CoreTypes.Console.RuntimeRole.TOOL.name) {
              // The placement for the navtree won't be "docked", so
              // go ahead and send signal that will either set the
              // placement to "floating" (visible = true), "minimized"
              // (visible = false).
              viewParams.signaling.navtreeToggled.dispatch("form-tabstrip", false);
            }
            else {
              // Send signal that will set placement to either "docked"
          ...
          .then(() => {
            ...
            $.getJSON({
              ...
              .then((pdjData) => {
                  ...
                  let nonwritable = pdjData.sliceForm.readOnly === true;
