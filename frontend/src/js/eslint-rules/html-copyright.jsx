...
const regexPatterns = {
  startLine: "^<\\!\\-\\-\\s*",
  copyrightLine: "\\s*Copyright\\s*\\(c\\)\\s*[\\s*\\d{4}$|\\d{4}$\\-\\d{4}$|\\d{4}$,]+[ ,|]\\s*Oracle[ |Corporation|Corp|Corp\\.]+and\\/or its affiliates.",
  uplLine: "\\s*The Universal Permissive License \\(UPL\\), Version 1\\.0",
  endLine: "\\s*\\-\\->"
};

const validCopyright = "<!--\n Copyright (c) 2020 Oracle Corporation and/or its affiliates.\n The Universal Permissive License (UPL), Version 1.0\n-->\n";

const messages = {
  "missingCopyright": "Oracle copyright missing from .html file.",
  "startLineError": "First line of .html file does not begin with <!--.",
  "copyrightLineError": "Copyright line is incorrect or in the wrong location. Change second line to 'Copyright (c) ${0} ${1} and/or its affiliates.' ${0} must be in ('2020', '2020,'). ${1} must be in ('Oracle', 'Oracle Corp', 'Oracle Corp.', 'Oracle Corporation').",
  "uplLineError": "Universal Permissive License (UPL) line is incorrect or in the wrong location. Change third line to ' The Universal Permissive License (UPL), Version 1.0'",
  "endLineError": "Last line of .html file is not -->, or is in the wrong location. Change fourth line to '*/'"
};
...
//        console.log(`filename=${filename}`);
        if (filename.indexOf(".htm") !== -1) results.push(filename);
      }
    }
  }
...
//      console.log(`lineKey=${actualRanges[i].key}, lineText=${lineText}`);
      if (!isLinePresent(actualRanges[i].key, lineText)) {
        reportEntries.push({loc: `${i + 1}:1`, errorMessage: messages[`${actualRanges[i].key}Error`]});
      }
    }
  }
...
// node src/js/eslint-rules/html-copyright.jsx --path src/js/views
// node src/js/eslint-rules/html-copyright.jsx --fix --path src/js/views
// node src/js/eslint-rules/html-copyright.jsx --path src/js/views/home.html
// node src/js/eslint-rules/html-copyright.jsx --fix --path src/js/views/home.html
// node src/js/eslint-rules/html-copyright.jsx -o html-copyright.html --path src/js/views
// node src/js/eslint-rules/html-copyright.jsx -o html-copyright.html --fix --path src/js/views/home.html
function getParams() {
  const argv = yargs
    .options({
      'fix': {describe: 'Fix by adding copyright to top of .html file.'},
      'path': {describe: 'Directory to start checking from, or an individual file to check.', demandOption: true},
      'output': {describe: 'Path to output .html file to write results in.', default: 'eslint-html.html', alias: 'o'}
    })
    .help()
    .alias('help', 'h')
    .alias('version', 'v')
...
function main() {
  ...
  console.log(`params.fix=${params.fix}, params.path=${params.path}, params.output=${params.output}`);
  const filenames = findFileNames(params.path, '.html');
  ...
  reportFeeder.entries.forEach((entry, index) => {
    ...
    if (problemsCount > 0) {
      entry.problems.forEach((problem) => {
        ...
        htmlTR += '<td>';
        htmlTR += '<a href="#">html-copyright.jsx</a>';
        htmlTR += '</td>';
        htmlTR += '</tr>';
      });
    }
...
function computeFixRanges(text) {
  ...
  solIndex = text.indexOf('Copyright (c) 2020 Oracle and/or its affiliates.');
  if (solIndex !== -1) eolIndex = text.indexOf('\n', solIndex);
  ranges.push(
    {
      key: "copyrightLine",
      text: "Copyright (c) 2020 Oracle Corporation and/or its affiliates.\n",
      solIndex: solIndex,
      eolIndex: eolIndex
    }
  );
...
function computeActualRanges(text) {
  ...
  if (solIndex !== -1) eolIndex = text.indexOf('\n', solIndex);
  ranges.push(
    {key: "startLine", text: `${text.substring(solIndex, eolIndex)}\n`, solIndex: solIndex, eolIndex: eolIndex}
  );
  ...
  if (solIndex !== -1) eolIndex = text.indexOf('\n', solIndex);
  ranges.push(
    {key: "uplLine", text: `${text.substring(solIndex, eolIndex)}\n`, solIndex: solIndex, eolIndex: eolIndex}
  );
