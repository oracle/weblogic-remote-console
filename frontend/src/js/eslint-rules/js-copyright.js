...
module.exports = {
  ...
  create(context) {
    ...
    function computeFixRanges(text) {
      ...
      if (solIndex !== -1 ) eolIndex = text.indexOf('\n', solIndex);
      ranges.push(
        {key: "licenseTag", text: " * @license\n", solIndex: solIndex, eolIndex: eolIndex}
      );

      solIndex = text.indexOf(' * Copyright (c) 2020 Oracle and/or its affiliates.');
      if (solIndex !== -1 ) eolIndex = text.indexOf('\n', solIndex);
      ranges.push(
        {key: "copyrightLine", text: " * Copyright (c) 2020 Oracle and/or its affiliates.\n", solIndex: solIndex, eolIndex: eolIndex}
      );

      solIndex = text.indexOf(' * The Universal Permissive License (UPL), Version 1.0');
      if (solIndex !== -1 ) eolIndex = text.indexOf('\n', solIndex);
      ranges.push(
        {key: "uplLine", text: " * The Universal Permissive License (UPL), Version 1.0\n", solIndex: solIndex, eolIndex: eolIndex}
      );

      solIndex = text.indexOf(' * @ignore');
      if (solIndex !== -1 ) eolIndex = text.indexOf('\n', solIndex);
      ranges.push(
        {key: "ignoreTag", text: " * @ignore\n", solIndex: solIndex, eolIndex: eolIndex}
      );
    ...
    function computeActualRanges(text) {
      ...
      if (solIndex !== -1 ) eolIndex = text.indexOf('\n', solIndex);
      ranges.push(
        {key: "licenseTag", text: `${text.substring(solIndex, eolIndex)}\n`, solIndex: solIndex, eolIndex: eolIndex}
      );
      ...
      if (solIndex !== -1 ) {
        ...
        if (eolIndex !== -1) {
          ranges.push(
            {key: "copyrightLine", text: `${text.substring(solIndex, eolIndex)}\n`, solIndex: solIndex, eolIndex: eolIndex}
          );
        }
      }
      else {
        ranges.push(
          {key: "copyrightLine", text: " * Copyright (c) 2020 Oracle and/or its affiliates.\n", solIndex: -1, eolIndex: -1}
        );
      }
      ...
      if (solIndex !== -1 ) eolIndex = text.indexOf('\n', solIndex);
      ranges.push(
        {key: "ignoreTag", text: `${text.substring(solIndex, eolIndex)}\n`, solIndex: solIndex, eolIndex: eolIndex}
      );
      ...
      Program() {
        ...
        if (!isLinePresent(fixRanges[0].key, lineText)) {
          ...
          context.report({
            ...
            fix: fixer => fixer.insertTextBeforeRange([0, 0], config.validCopyright)
          });
        }
