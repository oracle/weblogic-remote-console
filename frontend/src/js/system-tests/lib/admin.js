...
if (process.argv.length < 8) {
    ...
    console.log("Correct Syntax: 'mocha -g suite_test userName password remoteAdminUrl src/js/system/index.js' ")
    process.exit(1)
}
...
module.exports = function (driver, file) {
    ...
    let element;
    const pageobjects = {
        linkselenium: By.id('app-info-container')
    };
    return {
        ...
        selectDropDownList: async function (driver,idName,idSearchList,searchItem) {
            console.log("Select " +idSearchList+ " from " +idName+ " in " +searchItem+ " list.");
            element = await driver.findElement(By.id(idName));
            driver.executeScript("arguments[0].scrollIntoView({block:'center'})",element);
            await driver.sleep(300);
            await element.click();
