 ...
 * @module
 */
define(['../../core/adapters/file-adapter', './console-project', '../provider-management/data-provider-manager', '../provider-management/data-provider', '../../core/runtime', '../../core/utils', '../../core/types', '../../core/cfe-errors', 'ojs/ojlogger' ],
  function(FileAdapter, ConsoleProject, DataProviderManager, DataProvider, Runtime, CoreUtils, CoreTypes, CfeErrors, Logger){
    var projects = [];
...
//MLW            if (CoreUtils.isNotUndefinedNorNull(item.password)) dataProvider.putValue("password", item.password);
    ...
    return {
      ...
      loadConfigProjects: function() {
        const filepath = Runtime.getProperty(Runtime.PropertyName.CFE_STARTUP_CONFIG);
        return FileAdapter.readYaml(filepath)
          .then(configProjects => {
            if (CoreUtils.isNotUndefinedNorNull(configProjects)){
