...
define(['knockout', 'ojs/ojarraydataprovider', 'ojs/ojhtmlutils', 'ojs/ojknockout-keyset', '../../../cfe/utils/keyset-utils', '../../modules/change-manager', '../../../cfe/common/runtime', '../../../cfe/common/utils', 'ojs/ojlogger', 'ojs/ojaccordion', 'ojs/ojtable', 'ojs/ojbinddom', 'ojs/ojrowexpander'],
  function(ko, ArrayDataProvider, HtmlUtils, keySet, keySetUtils, ChangeManager, Runtime, Utils, Logger){
    function ShoppingCartTabTemplate(viewParams) {
      ...
      this.changeManagerSections = ko.observableArray([
        {id: ChangeManager.Section.CHANGE_MANAGER.name, label: "Change Manager", content: {}, count: ko.observable(0)},
        {id: ChangeManager.Section.ADDITIONS.name, label: "Additions", content: [], count: ko.observable(0)},
        {id: ChangeManager.Section.MODIFICATIONS.name, label: "Modifications", content: [], count: ko.observable(0)},
        {id: ChangeManager.Section.REMOVALS.name, label: "Removals", content: [], count: ko.observable(0)},
        {id: ChangeManager.Section.RESTART.name, label: "Restart", content: [], count: ko.observable(0)}
      ]);

      this.i18n = {
        tabstrip: {
          tabs: ko.observableArray([
            {id: "discard-tab-button", label: "Discard Changes", iconFile: "discard-changes-blk_24x24", disabled: false, visible: ko.observable(true) },
            {id: "commit-tab-button", label: "Commit Changes", iconFile: "commit-changes-blk_24x24", disabled: false, visible: ko.observable(true) }
          ])
        }
      };
      ...
      this.connected = function () {
        ...
        checkLockOwner();
//MLW        viewParams.signaling.perspectiveSelected.dispatch({id: "configuration", label: "Configuration"}, false);
      };
                ...
                if (typeof entry.oldValue !== "undefined") {
                  if (entry.oldValue === null)
                    ...
                    if (typeof entry.oldValue.value === 'object') {
                      entry.oldValue.value = Utils.getPropertiesDisplayValue(entry.oldValue.value);
                    }
                ...
                if (typeof entry.newValue !== "undefined") {
                  if (entry.newValue === null)
                    ...
                    if (typeof entry.newValue.value === 'object') {
                      entry.newValue.value = Utils.getPropertiesDisplayValue(entry.newValue.value);
                    }
