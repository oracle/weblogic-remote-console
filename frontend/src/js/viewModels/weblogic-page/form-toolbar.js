...
define(['knockout', "../../cfe/common/runtime", '../modules/change-manager', '../../cfe/services/perspective/perspective-memory-manager', '../modules/tooltip-helper', '../../cfe/common/utils', 'ojs/ojlogger', 'ojs/ojknockout'],
  function (ko, Runtime, ChangeManager, PerspectiveMemoryManager, TooltipHelper,  Utils, Logger) {
    function FormToolbar(viewParams) {
      ...
      this.i18n = {
        buttons: {
          "save": { id: "save", label: ko.observable("Save"), iconFile: ko.observable("save-icon-blk_24x24"), disabled: false },
          "new": { id: "new", label: "Create", iconFile: "add-icon-blk_24x24", disabled: false },
          "delete": { id: "delete", label: "Remove", iconFile: "remove-icon-blk_24x24", disabled: false },
          "back": { id: "back", label: "Back", iconFile: "back-icon-blk_24x24", disabled: ko.observable(true) },
          "next": { id: "next", label: "Next", iconFile: "next-icon-blk_24x24", disabled: ko.observable(false) },
          "finish": { id: "finish", label: "Create", iconFile: "add-icon-blk_24x24", disabled: ko.observable(true) }
        },
        icons: {
          "save": { id: "save", iconFile: "save-icon-blk_24x24", tooltip: "Save"},
          "create": { id: "create", iconFile: "add-icon-blk_24x24", tooltip: "Create"},
        ...
        menus: {
          shoppingcart: {
            "view": { id: "view", label: "View Changes", iconFile: "", disabled: false },
            "discard": { id: "discard", label: "Discard Changes", iconFile: "discard-changes-blk_24x24", visible: ko.observable(true), disabled: false},
            "commit": { id: "commit", label: "Commit Changes", iconFile: "commit-changes-blk_24x24", visible: ko.observable(true), disabled: false}
          }
        }
      };
      ...
      this.connected = function () {
        ...
        const formToolbarContainer = document.getElementById("form-toolbar-container");
        if (formToolbarContainer !== null) {
          new TooltipHelper(formToolbarContainer);
      ...
      this.shoppingCartMenuClickListener = function (event) {
        ...
        if (event.target.value === "view") {
          ...
          switch (event.target.value){
            case "commit":
              viewParams.onCommitDoTest().then(function (result) {
                if (result) {
      ...
      this.renderToolbarButtons = async function (eventType, hasNonReadOnlyFields) {
        if (self.perspective.id === "configuration" && !self.readonly()) {
          ...
          if (renderingInfo.kind === "creatableOptionalSingleton"){
            ...
            if (Utils.isUndefinedOrNull(viewParams.parentRouter.data.rdjData().data)) {
              display = "none";
            }
              ...
              .then((data) => {
                ...
                if (data.data.additions.length > 0) {
                  ...
                  if (typeof item !== "undefined") {
                    const identityPath = Utils.pathEncodedFromIdentity(item.identity);
      ...
      this.onSave = ko.observable(
        function (event) {
          ...
          viewParams.signaling.navtreeSelectionCleared.dispatch();
          self.changeManager(ChangeManager.getMostRecent());
          return viewParams.onSave("update");
        }
      );
