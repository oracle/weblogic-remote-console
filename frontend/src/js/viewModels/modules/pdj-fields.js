...
define(['knockout', 'ojs/ojarraydataprovider', 'ojs/ojlogger'],
    ...
    return {
      addFieldIcons: function (params) {
        ...
        if (params.restartRequired && !params.readOnly) {
          ...
          image.setAttribute("data-title", params.icons.restart.tooltip);
        ...
        moreIcon.setAttribute("data-title", params.icons.more.tooltip);
      ...
      addUploadFileElements: function (params) {
        ...
        image.setAttribute("data-title", params.choose.tooltip);
        ...
        image.setAttribute("data-title", params.clear.tooltip);
       ...
       */
      createSingleSelect: function (pdjTypes, value, dataValues, name, isEdit) {
        let optionsArray = [];
        let field = document.createElement('oj-select-single');

        field.classList.add("cfe-form-select-one");
        ...
        if (dataValues[name] != null && typeof dataValues[name].options !== 'undefined') {
          ...
          Logger.warn(`[FORM] createSingleSelect() used without options or legal values!`);
      ...
      createMultiSelect: function (dataValues, name) {
        ...
        field.setAttribute("available-header", "Available");
        field.setAttribute("chosen-header", "Chosen");
      ...
      getMultiSelectChosenItems: function (chosenItems, origChosenLabels) {
        ...
        if (!notDirty) result.data = { value: values };
      ...
      createDebugFlagsCheckboxset: function (properties, dataValues) {
        let options = [], debugFlagsEnabled = [];

        properties.forEach((property) => {
          if (dataValues[property.name] != null) {
            options.push({ value: property.name, label: property.label, enabled: dataValues[property.name].value });
        ...
        return {debugFlagsEnabled: debugFlagsEnabled, dataProvider: new ArrayDataProvider(options, { keyAttributes: 'value' }), field: field};
      },
      ...
      createInputText: function (className, params) {
        const field = document.createElement("oj-input-text")
        field.classList.add(className);
        field.setAttribute("placeholder", params["placeholder"]);
        return field;
      },

      createReadOnlyText: function (className) {
        const field = document.createElement("oj-input-text")
        field.classList.add(className);
        return field;
      },
      ...
      createTextArea: function (className, params) {
        const field = document.createElement("oj-text-area");
        field.classList.add(className);
        field.setAttribute("resize-behavior", params["resize-behavior"]);
        field.setAttribute("placeholder", params["placeholder"]);
        return field;
      },
