 ...
 * @return {Array}
 */
function convertPropertiesFileToJSObject(text, altBundle) {
  /**
  ...
  function convertPropertyToJSObject(property) {
    ...
    propNameParts.map((k, i, values) => {
      ...
      if (i === values.length - 1) {
        // Escape a double quote and remove a second apostrophe inserted by translation
        const valueReplaced = value.replace(/\"/g, '\\"').replace(/\'\'/g, '\'');
        unicodeString = decodeURIComponent(JSON.parse('"' + valueReplaced + '"'));
      }
      else {
        unicodeString = {};
      }
  ...
  // Filter the properties based on the prefix of the property key
  var filtered;
  if (!altBundle) {
    // Filter out property entries: wrc-electron, rch-ui, rch-signin
    filtered = properties.filter(property => {
      return (!property.startsWith('wrc-electron.')
              && !property.startsWith('rch-ui.')
              && !property.startsWith('rch-signin.'));
      });
  } else {
    // Keep only property entries that begin with the alternate bundle
    filtered = properties.filter(property => property.startsWith(`${altBundle}.`));
  }
 ...
 * @returns {Array}
 */
function getLocaleCodes(filenames, altBundle) {
  ...
  matched.forEach((match) => {
    ...
    if (!altBundle || !testLocales.includes(locale)) {
      localeCodes.push(locale);
    }
  });
  return localeCodes;
}
...
function createRootTranslationBundle(jsObjectDocument, localeCodes) {
  ...
  localeCodes.forEach((localeCode) => {
    // Test locales are not supported all others are supported
    jsObjectDocument[localeCode] = (!testLocales.includes(localeCode));
  });
 ...
 */
function createTranslationBundles(pathParam, outputParam, altBundle) {
  /**
   ...
   * @param {string} altBundle - Name of the altername bundle being created from the input file name
   ...
   */
  function getOutputFileSpec(outputFilePath, inputFileName, bundleName) {
    ...
    return {mkdirPath: pathParts, fileName: `${bundleName}.js`};
  }
  ...
  filenames.forEach((filename, index) => {
    ...
    translationBundle = convertPropertiesFileToJSObject(text, altBundle);
    ...
    const bundleName = (!altBundle ? 'frontend' : altBundle);
    const fileSpec = getOutputFileSpec(outputParam, filename, bundleName);
...
function getParams() {
  const argv = yargs
    .options({
      'path': {describe: 'Directory where resource bundles (.properties files) are located, or an individual resource bundle (.properties file) to use for input.', default: 'nls/frontend.properties', alias: 'p'},
      'output': {describe: 'Directory where translation bundles (.js files), or an individual translation bundle (.js file) is written to.', default: 'nls/frontend.js', alias: 'o'},
      'altbundle': {describe: 'Name of an alternate translation bundle to create at the output directrory.', default: null, alias: 'a'}
    })
    .help()
    .alias('help', 'h')
    .alias('version', 'v')
...
function main() {
  ...
  if (params.altbundle) {
    console.log(`params.altbundle=${params.altbundle}`);
  }
  let exitCode = 0;
  try {
    createTranslationBundles(params.path, params.output, params.altbundle);
  }
  catch(reason) {
...
// List of the locales used for testing
const testLocales = ["en-XA", "en-XC", "ar-XB"];

// node nls/create-translation-bundles.js -p nls -o web/js/resources/nls
// node nls/create-translation-bundles.js -o .
// node nls/create-translation-bundles.js -p nls
// node nls/create-translation-bundles.js -p nls/frontend.properties -o .
// node nls/create-translation-bundles.js -p nls/frontend_ar_XB.properties -o .
// node nls/create-translation-bundles.js -p nls/frontend.properties -o web/js/resources/nls
