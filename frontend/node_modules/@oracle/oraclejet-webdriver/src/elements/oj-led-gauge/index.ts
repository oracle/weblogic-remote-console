...
/**
 * Retrieve an instance of [OjLedGauge](../classes/elements.OjLedGauge.html).
 * @example
 * ```javascript
...
export async function ojLedGauge(driver: DriverLike, by: By): Promise<OjLedGauge> {
  ...
  if (!(webEl instanceof OjLedGauge)) {
    ...
    let supplemental = '';
    if (webEl.constructor.name === 'OjLedGauge') {
      supplemental = 'You likely have multiple installations of this package.';
    }
    throw Error(
      `ojLedGauge(${by}) created ${webEl.constructor.name} for <${tagName}>, but expected ${OjLedGauge.name}. ${supplemental}`
    );
  }
  return webEl;
}
