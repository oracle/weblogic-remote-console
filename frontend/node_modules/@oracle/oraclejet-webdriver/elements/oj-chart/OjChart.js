...
class OjChart extends OjChartBase_1.OjChartBase {
    ...
    async doGroupDrill(groupId) {
        ...
        if (drilling === 'on' || drilling === 'groupsOnly') {
            await this.getDriver().executeScript((chart, groupId) => chart.getAutomation().dispatchGroupDrill(groupId), this, groupId);
        }
    }
    /**
    ...
    async doSeriesDrill(seriesId) {
        ...
        if (drilling === 'on' || drilling === 'seriesOnly') {
            await this.getDriver().executeScript((chart, seriesId) => chart.getAutomation().dispatchSeriesDrill(seriesId), this, seriesId);
        }
    }
    /**
    ...
    async doItemDrill(seriesId, groupId, itemIndex) {
        ...
        if (drilling === 'on' || drilling === 'itemsOnly') {
            await this.getDriver().executeScript((chart, seriesId, groupId, itemIndex) => chart.getAutomation().dispatchItemDrill(seriesId, groupId, itemIndex), this, seriesId, groupId, itemIndex);
        }
    }
    /**
    ...
    async doMultiSeriesDrill() {
        ...
        if (multiSeriesDrill === 'on') {
            await this.getDriver().executeScript((chart) => chart.getAutomation().dispatchMultiSeriesDrill(), this);
        }
    }
}
