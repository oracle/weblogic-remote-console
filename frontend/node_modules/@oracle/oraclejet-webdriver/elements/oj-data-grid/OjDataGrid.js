...
class OjDataGrid extends OjDataGridBase_1.OjDataGridBase {
    ...
    async getCellContents(itemLocator) {
        ...
        for (let i = 0; i < cells.length; i++) {
            let cell = cells[i];
            cellMatch = await this.getDriver().executeScript((cell, rowIndex, columnIndex, rowKey, columnKey) => {
    ...
    async findCell(itemLocator) {
        ...
        const datagridCell = await this.getDriver().executeScript((element, rowIndex, columnIndex, rowKey, columnKey) => {
            ...
            for (let i = 0; i < cells.length; i++) {
                ...
                if (((rowIndex >= cellRowIndex && rowIndex < cellRowIndex + rowExtent) ||
                    cellRowKey === rowKey) &&
     ...
     * @override
     */
    async enterEditMode(itemLocator) {
        await this.whenReady();
        const datagridCell = await this.getDriver().executeScript((element, rowIndex, columnIndex, rowKey, columnKey) => {
            const cells = element.querySelectorAll('.oj-datagrid-cell');
            for (let i = 0; i < cells.length; i++) {
     ...
     * @override
     */
    async exitEditMode(exitParams) {
        await this.whenReady();
        const actions = this.getDriver().actions({ bridge: true });
        if (!exitParams || !exitParams.cancel) {
            await actions.sendKeys(selenium_webdriver_1.Key.ENTER).perform();
        }
        else {
    ...
    async getHeaderContents(subId, by) {
        ...
        for (let i = 0; i < headers.length; i++) {
            header = headers[i];
            headerMatch = await this.getDriver().executeScript((header, key, index, level) => {
    ...
    async findHeader(itemLocator) {
        await this.whenReady();
        const header = await this.getDriver().executeScript((element, axis, key, index, level) => {
        ...
        }, this, itemLocator.axis, itemLocator.key, itemLocator.index, itemLocator.level != null ? itemLocator.level : 0);
        if (header) {
    ...
    async getLabelContents(subId, by) {
        ...
        for (let i = 0; i < labels.length; i++) {
            label = labels[i];
            labelMatch = await this.getDriver().executeScript((label, level) => {
    ...
    async findLabel(itemLocator) {
        await this.whenReady();
        const headerLabel = await this.getDriver().executeScript((element, axis, level) => {
            const labels = element.querySelectorAll('.oj-datagrid-' + axis.replace('End', '-end') + '-header-label');
            for (let i = 0; i < labels.length; i++) {
        ...
        }, this, itemLocator.axis, itemLocator.level != null ? itemLocator.level : 0);
        if (headerLabel) {
