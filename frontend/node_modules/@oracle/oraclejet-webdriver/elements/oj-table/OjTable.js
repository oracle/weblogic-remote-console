...
class OjTable extends OjTableBase_1.OjTableBase {
    ...
    _changeScrollPosition(scrollPosition) {
    ...
    async findCell(itemLocator) {
        ...
        const el = await (0, oj_module_proxy_1.executeWithModules)(this.getDriver(), ['KeyUtils'], [this, itemLocator], ({ equals }, element, { rowIndex, columnIndex, rowKey, columnKey }) => {
            ...
            if (columns[columnIndex].template == null &&
    ...
    async findRow(itemLocator) {
        ...
        const el = await (0, oj_module_proxy_1.executeWithModules)(this.getDriver(), ['KeyUtils'], [this, itemLocator], ({ equals }, element, { rowIndex, rowKey }) => {
    ...
    async findHeader(itemLocator) {
        await this.whenReady();
        const el = await this.getDriver().executeScript((element, columnIndex, columnKey) => {
            const columns = element.columns;
            if (columnIndex == null) {
                for (let i = 0; i < columns.length; i++) {
                    if (columns[i].id === columnKey) {
    ...
    async doSort(header, direction) {
        await this.whenReady();
        const msg = await (0, oj_module_proxy_1.executeWithModules)(this.getDriver(), ['CustomElementUtils'], [this, header, direction], ({ getElementBridge }, element, headerId, sortDirection) => {
     ...
     * @typeparam K Type of keys
     */
    async changeSelectedKeys(selected) {
        await this.whenReady();
