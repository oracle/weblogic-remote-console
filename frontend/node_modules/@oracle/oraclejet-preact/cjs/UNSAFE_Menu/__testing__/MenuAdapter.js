...
class MenuAdapter extends UNSAFE_TestAdapter.TestAdapter {
    /**
    ...
    async _doMenuActionClick(path) {
        ...
        for (const [index, identifier] of Object.entries(arraySpecificPath)) {
            ...
            await UNSAFE_Driver.waitFor(async () => {
                ...
                return !(await menu.waitForElements(UNSAFE_Locators.byCss(`[oj-menu-skeleton]`))).length;
            });
    ...
    async _getMenuElements() {
        ...
        return this.getTestDriver().waitForElements(UNSAFE_Locators.byCss(`[data-oj-layer]:has([data-testid=${testId}]) [role=menu]`));
    }
    /**
    ...
    async _clickingMenuItem(query, searchElement, errMessage) {
        try {
            const item = await searchElement.waitForElement(UNSAFE_Locators.byCss(query));
...
exports.MenuAdapter = MenuAdapter;
