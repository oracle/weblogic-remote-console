...
function useDraggable({ isDisabled = false, containerRef, onDragMove, onDragStart, onDragEnd }) {
    const isDragging = hooks.useRef(false);
    const startPosition = hooks.useRef();
    const offset = hooks.useRef({ x: 0, y: 0 });
    const initialMousePosition = hooks.useRef({ x: 0, y: 0 });
    const initialClientRect = hooks.useRef({ x: 0, y: 0, width: 0, height: 0 });
    const constrainPos = hooks.useCallback((x, y) => {
     ...
     */
    const handleDrag = hooks.useCallback((event) => {
     ...
     */
    const endDrag = hooks.useCallback((event) => {
        if (!isDragging.current || !startPosition.current)
            return;
     ...
     */
    const startDrag = hooks.useCallback((event) => {
     ...
     */
    const mouseEnter = hooks.useCallback((event) => {
        if (isDisabled)
            return;
        const elem = event.currentTarget;
        if (elem) {
     ...
     */
    const mouseLeave = hooks.useCallback((event) => {
        if (isDisabled)
            return;
        const elem = event.currentTarget;
        if (elem) {
...
//# sourceMappingURL=useDraggable-aa4ac3a2.js.map
