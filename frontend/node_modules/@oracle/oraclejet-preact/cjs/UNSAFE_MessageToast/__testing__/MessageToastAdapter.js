...
class MessageToastAdapter extends UNSAFE_TestAdapter.TestAdapter {
    /**
    ...
    async doClose(messageLocator) {
        ...
        let closeButton;
        try {
            closeButton = await (await this.getElement()).waitForElement(UNSAFE_Locators.byCss(`${toastSelector} ${closeButtonSelector}`));
        }
        catch {
    ...
    async findDetail(messageLocator) {
        ...
        try {
            const customDetail = await (await this.getElement()).waitForElement(UNSAFE_Locators.byCss(`${toastSelector} ${customDetailContentSelector}`));
            return customDetail;
        }
        catch {
    ...
    async findIcon(messageLocator) {
        ...
        try {
            const customIcon = await (await this.getElement()).waitForElement(UNSAFE_Locators.byCss(`${toastSelector} ${customIconContentSelector}`));
            return customIcon;
        }
        catch {
...
exports.MessageToastAdapter = MessageToastAdapter;
