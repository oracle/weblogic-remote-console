...
class MessageBannerAdapter extends TestAdapter {
    /**
    ...
    async doClose(messageLocator) {
        ...
        let closeButton;
        try {
            closeButton = await (await this.getElement()).waitForElement(byCss(`${messageSelector} ${closeButtonSelector}`));
        }
        catch {
    ...
    async findDetail(messageLocator) {
        ...
        try {
            const detail = await (await this.getElement()).waitForElement(byCss(`${messageSelector} ${customDetailContentSelector}`));
            return detail;
        }
        catch {
