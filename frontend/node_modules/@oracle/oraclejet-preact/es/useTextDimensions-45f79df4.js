...
const toFontString = (font) => {
    ...
    if (lineHeight === 'normal') {
        // Starting in Safari 18.4, if line-height is "normal" in the font string, CanvasRenderingContext2D fails to parse it.
        // See https://bugs.webkit.org/show_bug.cgi?id=291667 and JET-73602
        // Workaround is to simply omit the "normal" value, since that's the default anyway:
        return `${fontStyle} ${fontVariant} ${fontWeight} ${fontStretch} ${fontSizeString} ${fontFamily}`;
    }
...
const useTextDimensions = ({ backend = 'auto' } = {}) => {
    ...
    const { onBusyStart, onBusyEnd } = useAddBusyState('useTextDimensions: document fonts');
    useLayoutEffect(() => {
    ...
    }, [backendConfig, onBusyStart, onBusyEnd]);
    return {
...
//# sourceMappingURL=useTextDimensions-45f79df4.js.map
