...
import { useLayoutEffect, useCallback, useRef, useState, useMemo, useEffect, useImperativeHandle } from 'preact/hooks';
...
import { L as LOADMORE_STYLE_CLASS, a as LoadMoreCollection } from './LoadMoreCollection-02671d0d.js';
...
import { S as Selector } from './Selector-1ceeaf0a.js';
import { tableRowStylesHC, tableRowStyles } from './PRIVATE_Table/themes/TableRowStyles.css.js';
import { e as isSameKey, c as containsKey, a as addKey, r as removeKey } from './keys-49c92dbb.js';
import { F as FormContext } from './FormContext-55fb8000.js';
import { g as getClientHints } from './clientHints-a42b510c.js';
import { textAlignStyles, verticalAlignStyles, preventBrowserTooltipStyle, backgroundStyles, tableHeaderCellTextStyles, tableHeaderSortContainerStyles, justifyContentStyles, tableHeaderRowStyles, tableFooterRowStyles, tableDragIndicatorStyles, scrollerStyles, innerTableStyles } from './PRIVATE_Table/themes/Table.css.js';
...
import { k as keyExtractor, f as findElementByKey, g as getElementContainsFunc, n as handleEnterActionableMode, o as handleWrapActionableFocus } from './collectionUtils-893f2808.js';
...
import { S as SelectorAll } from './SelectorAll-d151b011.js';
import { S as SortControl } from './SortControl-16040b60.js';
import { S as SortableContext } from './useSortableContext-88a0fc5a.js';
import { B as Button } from './Button-8dbdb872.js';
import { D as Dialog } from './Dialog-06e086a5.js';
import { F as Flex } from './Flex-5befe411.js';
import { M as Menu, a as MenuItem } from './SelectMenuGroupContext-0c1a7603.js';
import { S as Submenu, a as SelectSingleMenuGroup, b as SelectMenuItem } from './Submenu-712f5f55.js';
...
import { N as NumberInputText } from './NumberInputText-9c8015e1.js';
import { F as FocusTracker } from './FocusTracker-02ad6723.js';
...
import { u as useContextMenuGesture } from './useContextMenuGesture-906cf78d.js';
import { u as useColorScheme } from './useColorScheme-96cce6a1.js';
import { u as useScale } from './useScale-0ebe15ed.js';
import { T as TooltipContent } from './useTooltipControlled-d7724623.js';
import { S as SHOW_TOOLTIP_DELAY, g as getWrappedTooltipContent, H as HIDE_TOOLTIP_DELAY, a as getGlobalTooltipManager } from './tooltipUtils-466c63c4.js';
import { a as allTabbableElements, g as getActiveElement, c as getBodyElement, i as isTabbableElement } from './tabbableUtils-ce19b388.js';
...
function TableSkeletonRow({ tableId, colspan, hasTabIndex, isShowFocusRing, isPendingLayout = false, isLoadMore = false, isHighContrast }) {
    ...
    return (jsx("tr", { class: LOADMORE_STYLE_CLASS, role: 'row', children: jsx("td", { class: cellClasses, colSpan: colspan, role: 'gridcell', tabIndex: hasTabIndex ? 0 : -1, "data-oj-cell-type": isPendingLayout ? 'pending' : isLoadMore ? 'loadMore' : 'loading', ...(isPendingLayout
                ...
                : isLoadMore
                    ...
                    : { 'data-oj-table-loading-cell': tableId }), "data-oj-table-focusable": tableId, children: jsxs("div", { class: innerCellClasses, children: [accCellContent, [...Array(isLoadMore ? 3 : 25)].map((_element, index) => (jsx("div", { class: index === 0 ? firstRowClasses : rowClasses, children: jsx(TableSkeleton, { height: "4x", isHighContrast: isHighContrast }) })))] }) }) }));
}
...
function TableRow({ tableId, rowKey, rowData, rowIndex, isFinalRow, columnsArray, rowHeaderColumnKeys, hasVerticalGridlines, hasHorizontalGridlines, hasFooters, hasVerticalUnderflow, hasMore, isSelected, isPreviousSelected, isRowSelectionEnabled, isSingleRowSelectionEnabled, selectedRowKeys = { all: false, keys: new Set() }, isShowFocusRing, activeColumnKey, isTabbableMode, selectedColumnKeys = { all: false, keys: new Set() }, variant, disabledInteractionKeys, onSelectionChange, startStickyEdge, endStickyEdge, firstStickyColumnIndex, finalStickyColumnIndex, columnBorderEdges, isRtl, isHighContrast }) {
    ...
    return (jsx("tr", { "aria-rowindex": rowIndex + 2, class: classes, role: 'row', "data-oj-key": rowKey, "data-oj-table-data-row": tableId, ...(isRowSelectionEnabled
            ? isSelected
                ? { 'aria-selected': true }
                : { 'aria-selected': false }
...
function TableBody({ rootRef, viewportInfo, tableId, isValidRender, getAccessibleRowHeaders, tableData, columnsArray, hasVerticalGridlines, hasHorizontalGridlines, hasFooters, hasVerticalUnderflow, isShowFocusRing, isTabbableMode, hasMore, onLoadMore, activeCell, currentRowKey, currentRowVariant, selected, isRowSelectionEnabled, isSingleRowSelectionEnabled, disabledInteractionKeys, onSelectionChange, noDataRenderer, startStickyEdge, endStickyEdge, firstStickyColumnIndex, finalStickyColumnIndex, columnBorderEdges, isRtl, isHighContrast }) {
    ...
    return (jsx("tbody", { class: backgroundStyles.inherit, role: 'presentation', "data-oj-table-body": tableId, children: isShowNoData || !isValidRender ? (jsx(TableNoData, { ...getNoDataProps() })) : (jsxs(Fragment, { children: [jsx(LoadMoreCollection, { data: tableData, loadMoreIndicator: jsx(TableSkeletonRow, { tableId: tableId, hasTabIndex: isShowEmptyLoading
                            ...
                            (isShowEmptyLoading
                                ? activeCell?.type === 'loading'
                                : activeCell?.type === 'loadMore'), isHighContrast: isHighContrast }), hasMore: hasMore, onLoadMore: onLoadMore, viewportConfig: loadMoreViewportConfig, children: collectionChildrenFunc }), jsx("tr", { style: browser !== 'firefox' ? 'height: 100%;' : undefined, role: 'presentation', "data-oj-table-spacer-row": tableId })] })) }));
}
...
function TableContextMenu({ contextMenuConfig, contextMenuContext, ...menuProps }) {
    ...
    return contextMenuConfig.itemsRenderer(contextMenuContext) != null ? (jsx(Menu, { "aria-label": accessibleLabel, ...menuProps, children: menuProps.isOpen && contextMenuConfig.itemsRenderer(contextMenuContext) })) : (jsx(Fragment$1, {}));
}
/**
...
function applyScrollDetail(rootElement, scrollerElement, viewportInfo, tableId, columnsArray, stickyColumnInfo, sizingInfo, scrollDetail, isRtl, appliedColumnWidths) {
    if (scrollDetail.cell.type === 'data') {
        const rowElement = findElementByKey(rootElement, scrollDetail.cell.rowKey, `[data-oj-table-data-row='${tableId}']`);
...
function getCellContextMenuContextFromCell(tableData, columnsArray, setIsResizeDialogOpen, cell, sortCriterion, onSortCriterionChange) {
    if (cell?.type === 'data' || cell?.type === 'header' || cell?.type === 'footer') {
        ...
        const defaultMenuItems = hasResizableColumns || hasSortableColumns ? (jsxs(Fragment$1, { children: [hasResizableColumns ? allMenuItems['resizeColumn'] : undefined, hasSortableColumns ? allMenuItems['sortColumn'] : undefined] })) : undefined;
        if (cell.type === 'data') {
...
function handleAdjacentRowGesture(rootElement, tableId, columnsArray, initialCell, onActiveCellChanged, isPrevious, isExtendSelectionGesture, hideTooltip, updateDeferredScrollDetail) {
    ...
    if (cellType === 'noData') {
        ...
        if (cellType === 'data') {
            ...
            const currentRowElement = findElementByKey(rootElement, currentRowKey, `[data-oj-table-data-row='${tableId}']`);
...
function useSizingManager({ rootRef, innerTableRef, tableId, tableData, viewportConfig, externalScrollerRef, layout, columns, columnOrder, columnWidths, isRtl, isHighContrast }) {
    ...
    useImperativeHandle(externalScrollerRef, () => {
    ...
    const _updateStickyEdgesFromResize = useCallback((scrollerElement) => {
        if (appliedColumnWidths != null) {
...
export { Table as T };
//# sourceMappingURL=Table-f8530493.js.map
