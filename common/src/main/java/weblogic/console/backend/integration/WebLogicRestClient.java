...
import weblogic.console.backend.ConsoleBackendRuntime.State;
...
public class WebLogicRestClient {
  ...
  // We get the earliest one that we support so that we get all the properties, whether deprecated or not
  private static final String WEBLOGIC_REST_URN = "/management/weblogic/12.2.1.0.0";
  ...
  public static EventSource getEventSource(
    ...
    return EventSource.target(getWebTarget(request)).named(name).build();
  }
   ...
   * @throws WebLogicRestClientException
   */
  public static Response post(WebLogicRestRequest request, Entity entity) throws WebLogicRestClientException {
    WebTarget webTarget = getWebTarget(request);
    MultivaluedMap<String, Object> headers = WebLogicRestClientHelper.createHeaders(request);
    Response response;
  ...
  private static String calculateTargetUri(WebLogicRestRequest request) {
    ...
    if (request.serverUrl() != null) {
      calculatedTargetUri = request.serverUrl() + WEBLOGIC_REST_URN;
      LOGGER.finest("Request Server URL calculatedTargetUri=" + calculatedTargetUri);
    } else if (request.connection() != null) {
      calculatedTargetUri =
        request.connection().getDomainUrl() + WEBLOGIC_REST_URN;
      LOGGER.finest("Request Connection URL calculatedTargetUri=" + calculatedTargetUri);
    } else if (State.CONNECTED == ConsoleBackendRuntime.INSTANCE.getState()) {
      Connection connection = ConsoleBackendRuntime.INSTANCE.getConnection();
      if (connection != null) {
        calculatedTargetUri = connection.getDomainUrl() + WEBLOGIC_REST_URN;
        LOGGER.finest(
